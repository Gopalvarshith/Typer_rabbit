...
def speech_to_text_google(audio_file):
    """Convert speech to text using Google Speech API (requires GOOGLE_SPEECH_API_KEY)."""
    api_key = os.environ.get("GOOGLE_SPEECH_API_KEY", "").strip()
    if not api_key:
        print(
            "Error: Set environment variable GOOGLE_SPEECH_API_KEY to your "
            "Google Cloud / Speech API key (do not commit keys to the repo)."
        )
        return None
    try:
        # Read audio file
        ...
        url = f"http://www.google.com/speech-api/v2/recognize?{q}"
